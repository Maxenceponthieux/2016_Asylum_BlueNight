    ...
    public abstract class TrailRenderer_Base : MonoBehaviour
    {
        ...
        private void GenerateMesh(PCTrail trail)
        {
            ...
            for (int i = 0; i < trail.Points.Count; i++)
            {
                ...
                {
					dir = (trail.Points[i + 1].Position - p.Position).normalized;
					if( Vector3.Magnitude( dir ) > 0.5f )
					{
						cross = Vector3.Cross(dir, camForward).normalized;
					}
					else
					{
						//Debug.Log( cross + " index:" + (i+1) + " " + trail.Points[i + 1].Position  + " index:" + i + " " +  p.Position);
					}
                }
                else
                {
					dir = (p.Position - trail.Points[i - 1].Position).normalized;
					...
					{
						//Debug.Log( cross  + " index:" + i + " " + p.Position  + " index:" + (i-1) + " " + trail.Points[i - 1].Position);
					}
                }
